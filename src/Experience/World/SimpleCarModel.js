...
export default class SimpleCarModel {
    constructor() {
        ...
        this.currentDesign = 'Kart' // default design

        // chassis mesh group (keep same reference so other code can hold it)
    ...
    setModel() {
        ...
        // Rebuild into existing chassisMesh group (preserve reference)
        while (this.chassisMesh.children.length) {
            const child = this.chassisMesh.children[0]
            this.chassisMesh.remove(child)
        }

        if (this.currentDesign === 'Kart') {
            // === KART BODY ===
            ...
            // Remove previous wheels from scene if present
            if (this.frontWheelMesh1 && this.scene.children.includes(this.frontWheelMesh1)) this.scene.remove(this.frontWheelMesh1)
            if (this.frontWheelMesh2 && this.scene.children.includes(this.frontWheelMesh2)) this.scene.remove(this.frontWheelMesh2)
            if (this.rearWheelMesh1 && this.scene.children.includes(this.rearWheelMesh1)) this.scene.remove(this.rearWheelMesh1)
            if (this.rearWheelMesh2 && this.scene.children.includes(this.rearWheelMesh2)) this.scene.remove(this.rearWheelMesh2)
        ...
        } else if (this.currentDesign === 'Cat') {
            // === CAT BODY ===
            const catColor = 0xFFCC99
            const bodyMaterial = new THREE.MeshToonMaterial({ color: catColor })

            const bodyGeometry = new THREE.BoxGeometry(1.6, 0.7, 2.6)
            const bodyMesh = new THREE.Mesh(bodyGeometry, bodyMaterial)
            bodyMesh.position.y = 0.25
            bodyMesh.castShadow = true
            this.chassisMesh.add(bodyMesh)
            ...
            // Head (front)
            const headGeometry = new THREE.BoxGeometry(1.0, 0.8, 0.9)
            const headMaterial = new THREE.MeshToonMaterial({ color: 0xFFDDCC })
            const head = new THREE.Mesh(headGeometry, headMaterial)
            head.position.set(0, 0.45, 1.2)
            head.castShadow = true
            this.chassisMesh.add(head)

            // Ears
            const earGeometry = new THREE.ConeGeometry(0.15, 0.35, 6)
            const earL = new THREE.Mesh(earGeometry, headMaterial)
            earL.position.set(-0.35, 0.95, 1.05)
            earL.rotation.x = Math.PI
            this.chassisMesh.add(earL)
            const earR = earL.clone()
            earR.position.x = 0.35
            this.chassisMesh.add(earR)

            // Eyes
            const eyeGeo = new THREE.SphereGeometry(0.08, 8, 8)
            const eyeMat = new THREE.MeshToonMaterial({ color: 0x000000 })
            const eyeL = new THREE.Mesh(eyeGeo, eyeMat)
            eyeL.position.set(-0.25, 0.5, 1.6)
            this.chassisMesh.add(eyeL)
            const eyeR = eyeL.clone()
            eyeR.position.x = 0.25
            this.chassisMesh.add(eyeR)

            // Nose
            const noseGeo = new THREE.BoxGeometry(0.12, 0.08, 0.06)
            const nose = new THREE.Mesh(noseGeo, new THREE.MeshToonMaterial({ color: 0xFF5577 }))
            nose.position.set(0, 0.42, 1.85)
            this.chassisMesh.add(nose)

            // Tail
            const tailGeo = new THREE.CylinderGeometry(0.05, 0.05, 0.9, 8)
            ...
            // Seat and accents
            const seatGeometry = new THREE.BoxGeometry(0.7, 0.35, 0.5)
            const seatMaterial = new THREE.MeshToonMaterial({ color: 0x222222 })
            const seat = new THREE.Mesh(seatGeometry, seatMaterial)
            ...
            this.bodyMaterial = bodyMaterial
            this.wingMaterial = bodyMaterial

            // Create and add wheels for cat design (slightly smaller)
            if (this.frontWheelMesh1 && this.scene.children.includes(this.frontWheelMesh1)) this.scene.remove(this.frontWheelMesh1)
            if (this.frontWheelMesh2 && this.scene.children.includes(this.frontWheelMesh2)) this.scene.remove(this.frontWheelMesh2)
            if (this.rearWheelMesh1 && this.scene.children.includes(this.rearWheelMesh1)) this.scene.remove(this.rearWheelMesh1)
            if (this.rearWheelMesh2 && this.scene.children.includes(this.rearWheelMesh2)) this.scene.remove(this.rearWheelMesh2)

            this.frontWheelMesh1 = this.createKartWheel(0.45, 0.35, catColor)
            this.frontWheelMesh1.position.set(5 + 1.3, 5 - 0.2, 1.5)
            this.frontWheelMesh2 = this.createKartWheel(0.45, 0.35, catColor)
            this.frontWheelMesh2.position.set(5 - 1.3, 5 - 0.2, 1.5)
            this.rearWheelMesh1 = this.createKartWheel(0.45, 0.35, catColor)
            this.rearWheelMesh1.position.set(5 + 1.3, 5 - 0.2, -1.3)
            this.rearWheelMesh2 = this.createKartWheel(0.45, 0.35, catColor)
            this.rearWheelMesh2.position.set(5 - 1.3, 5 - 0.2, -1.3)
    ...
    // Set a new design and rebuild the model
    setDesign(design) {
        if (!design) return
